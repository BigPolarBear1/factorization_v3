...
###To run: python3 run_qs.py -base 6000 -keysize 200 -debug 1 -lin_size 100_000 -quad_size 1
...
cdef solve_roots(long long prime,long long n):
    
    cdef Py_ssize_t iN=0      
    temp_hmap = array.array('Q',[0]*prime*2)
    temp_hmap[0]=1
    modi=modinv(4*n,prime)
    cdef long long new_square
    while iN < prime:
...
@cython.boundscheck(False)
@cython.wraparound(False)
cdef factorise_fast(value,factor_base):
    ...
    while value % 2 == 0:
        ...
        value //= 2
    #cdef int factor 
    cdef Py_ssize_t length=len(factor_base)#factor_base[0]
    cdef Py_ssize_t i=0
    while i < length:
...
@cython.boundscheck(False)
@cython.wraparound(False)
cdef void miniloop_non_simd(Py_ssize_t dist1,unsigned int [::1] temp,Py_ssize_t prime,unsigned int log,Py_ssize_t size):
    while dist1 < size:
        temp[dist1]+=log
        dist1+=prime
    return
...
@cython.boundscheck(False)
@cython.wraparound(False)
cdef construct_interval_2(quad_co,lin_co,cmod,primeslist,hmap,n,indexmap,unsigned int [::1] temp,logmap,size,lprimes):
   # print(primeslist)
    cdef Py_ssize_t i=0
    cdef Py_ssize_t k
   # print(primeslist)
    #print("indexmap: ",indexmap)
    cdef Py_ssize_t length = len(lprimes)#quad_interval[0]
    local_primes=[]
    while i < length:
        prime_index=lprimes[i]
        prime=primeslist[prime_index]#quad_interval[i]

        new_square=(quad_co*4*n)%prime
        test=jacobi(new_square,prime)
        if test != 1:
            print("WTF")
            time.sleep(10000)
        #    i+=1
         #   continue
        local_primes.append(prime)
      #  prime_index=quad_interval_index[i]
        lcmod=cmod%prime
        if prime<min_prime or lcmod ==0: 
            i+=1
            continue
        ...
        miniloop_non_simd(dist1,temp,prime,log,size)
        if dist1 != dist2:
...
#@cython.boundscheck(False)
#@cython.wraparound(False)
cdef sieve_quads(cfact,n,local_mod,quad_interval,quad_interval_index):
    found=[]
    local_primes=[]
    i=0
    while i < len(quad_interval):
        ...
        while j < length:
            if quad_interval[i][j] in cfact:
                ...
                local_primes[-1].append(quad_interval_index[i][j])  ###To do: Since we're dealing with primes, we can optimize this using a unique set... 
            j+=1
        if hit == len(cfact):
            found.append(i+1)
        i+=1
...
cdef get_lin(found_quads,hmap,indexmap,cfact,local_mod,indexes):
    a=[]
    all_lin_parts=[]
    i=0
    while i < len(found_quads):
        all_lin_parts.append([])
        j=0
        lin=0
        while j < len(indexes):
            ind=indexes[j]
            prime=cfact[j]
            ind2=indexmap[ind][found_quads[i]%prime]
            ...
            lin+=aq*gamma
            all_lin_parts[-1].append(aq*gamma)
            j+=1
        lin%=local_mod
        a.append(lin)
        i+=1
    return a,all_lin_parts
    ...
    while len(ret_array[0]) < round((base+1)*matrix_mul):
        ...
        while i < 1:#(rstop-1):
            ...
            if local_mod>1:
                if len(cfact)<2:
                    break
                found_quads,lprimes=sieve_quads(cfact,n,local_mod,quad_interval,quad_interval_index)  ###TO DO: Since we refresh the modulus, we should do one at a time here.. 
                founds_lins,all_lin_parts=get_lin(found_quads,hmap,indexmap,cfact,local_mod,indexes)  ###TO DO: Since we refresh the modulus, we should do one at a time here.. 
                c=0
                while c < len(found_quads):
                    lin=founds_lins[c]
                    quad=found_quads[c]
                    lin_parts=all_lin_parts[c]
                    quad%=local_mod   
                    lin%=local_mod
                    if quad == 0 or lin == 0:
                        continue  
                    ...
                    while poly_ind < end:
                        ...
                        single_interval,local_primes=construct_interval_2(quad,lin,local_mod,primeslist,hmap,n,indexmap,target,logmap,size,lprimes[quad-1])
                        local_primes.extend(cfact)
                        ...
                        if mod_found > mod_refresh:  ##I don't know what the optimal value is here? 
...
#@cython.boundscheck(False)
#@cython.wraparound(False)
cdef process_interval(ret_array,unsigned int [::1] interval,n,quad_co,lin_co,partials, large_prime_bound,local_primes,int threshold,cmod,Py_ssize_t size,mod_found):
...
def find_comb(n,hmap,primeslist1,indexmap):
   ...
   # residue_map=[]
    ...
    ret_array=construct_interval(ret_array,partials,n,primeslist1,hmap,gathered_quad_interval,gathered_ql_interval,1,quad_sieve_size+1,quad_interval,quad_interval_index,threshold_map,indexmap,seen,large_prime_bound,tmul,tnum_list,lprimes_list,tnum_bit_list,logmap)
