...
###WORK IN PROGRESS!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
###Factorization_v3 minor_version: 036
###notes: use with python3 QSv3_036.py -keysize 120


##Optimized building of the iN map and parts of the lifting code. I'll refactor the entire smooth finding logic next.. the idea is good, but its poorly implemented.
...
def lift_collected_p(prime,n,collected,z,sqlimit):
    ...
    while 1:
        ...
        while i < len(rt):
            ...
            if len(ret)>0:
                if ret[0][0] > sqlimit and (prime**exp)-ret[0][0] > sqlimit:
                    skip=1
                    break
                all_ret.extend(ret)
            else:
...
def lift_collected(collected,n,z,sqlimit):
    ...
    while i< len(collected):
        ...
        if len(ret)==0:
            add=1
            a=z
            while a%collected[i]== 0:
                a//=collected[i]
                add*=collected[i]
            ret=[add,collected[i+1]]
           # print("ret: ",ret)
           # time.sleep(1000)
        all_ret.extend(ret)
        i+=2
...
def create_interval(co,cur_mod,curr,collected,lcol,sieve_len,limit):
    ...
    ##To do2: This is all wrong. Redo everything. The way we do things now, we create to much duplication. Should just create one hashmap per iN value.
    i=0
    while i < len(lcol):
        j=0
        if i == curr:
        ...
        while j < len(lcol[i+1]):
            ...
            while mod < lcol[i]+1:
                ...
                while co2 < limit:
                    try:
                        test=hmap[str(co2)]
                        test[1].append(collected[i]) 
                        test[0]*=collected[i]                  
                    except Exception as e:
                        b=[cur_mod,collected[i]]
                        hmap[str(co2)]=[cur_mod*collected[i],b] 
                    co2+=inc
                mod*=collected[i]
            j+=1
        i+=2
...
def check_4_smooths(hmap,n,x,primeslist1):
    ...
    for i,(k,v) in enumerate(hmap.items()):
        ...
        smooth_can//=v[0]
       # print("smooth_can: ",smooth_can)
        test=math.log2(smooth_can)
        if test.is_integer() == 1: ###To do: I need to fix some things later with calculating lifting results when the gcd  isnt 1 with the quadratic coefficient
            s,c,f=test_smooth_can(co,n,x,primeslist1) ##To do: can remove this now.
            if s !=0:
                smooths.append(s)
                xlist.append(c)
                faclist.append(f)
