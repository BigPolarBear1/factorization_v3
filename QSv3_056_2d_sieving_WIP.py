...
###notes: use with python3 QSv3_056_2d_sieving_WIP.py -base 1000 -keysize 120
...
##To do: Since currenty each process is sieving over multiple quadratic coefficient ranges, perhaps it is better to move paralellising to the sieving process itself, since we want to keep our quad_co range fairly small. The process at index 0 now finds many more smooths. 
...
workers=4 #max amount of parallel processes to use
...
def construct_interval_1d(quad_co,lin_co,cmod,primeslist,hmap,log_map):
    interval=[0]*lin_sieve_size
    primeslist_len=len(primeslist)
    i=0
    while i < primeslist_len:
        prime=primeslist[i]
        ...
        while j < len(hmap[i][0]):
            if hmap[i][0][j][0]==quad_co%prime:
                co2=hmap[i][0][j][1]
                dist1=solve_lin_con(cmod,co2-lin_co,prime)
                co2=(prime-co2)%prime
                dist2=solve_lin_con(cmod,co2-lin_co,prime)
                if dist1 < len(interval):
                    x=dist1

                    while x <len(interval):
                        interval[x]+=log#round(math.log2(primeslist[i]))
                        x+=prime
...
def construct_interval(n,cmod,cfact,qli,primeslist,hmap,gathered_quad_interval,gathered_ql_interval,rstart,log_map,quad_threshold):
    all_interval=[]
    quad_co_list=[]
    index_list=[]
    lin_co_list=[]
    ...
    while i < gathered_quad_interval_len:
        local_mod=gathered_quad_interval[i]
        if local_mod>quad_threshold:
            ...
            while j < len(lin_list):
                ...
                j+=2
            lin%=local_mod
            if (lin**2+4*n*quad)%local_mod !=0: ##To do: Delete for final version
                print("WTF ERROR")
                time.sleep(10000)
            quad_co_list.append(quad)
            index_list.append(i)
            lin_co_list.append(lin)
            interval=construct_interval_1d(quad,lin,local_mod,primeslist,hmap,log_map)
            all_interval.append(interval)
        i+=1
    return all_interval,quad_co_list,lin_co_list,index_list,mod_list

def process_interval(procnum,return_dict,interval,n,quad_co_list,lin_co_list,primeslist,ret_array,cfact,partials,large_prime_bound,quad_interval,index_list,threshold_map,mod_list):
    i=0
    while i < len(interval):
        j=0
        cmod=mod_list[i]
        threshold = threshold_map[i] 
        interval_len=len(interval[i])
        local_primes=set(quad_interval[index_list[i]])
...
def construct_quad_interval(hmap,primeslist1,rstart,rstop,n):
    ...
    while i < rstop:
        ...
        threshold = int(math.log2(lin_sieve_size * math.sqrt(n*4*i)) - thresvar)
        threshold_map.append(threshold)
        i+=1
    i=0
    while i < len(hmap):
        log_map.append(math.log2(primeslist1[i]))
        j=0
        while j < len(hmap[i][0]):
            x=hmap[i][0][j][0]
            ...
            j+=1
        i+=1
    return quad_interval,threshold_map,log_map
...
def find_comb(n,procnum,return_dict,rstart,rstop,hmap,primeslist1):
    ...
    seen=[]
    start=default_timer()
    quad_interval,threshold_map,log_map=construct_quad_interval(hmap,primeslist1,rstart,rstop,n)
    if g_debug > 0:
        duration = default_timer() - start
    ...
    max_bits_inc=50
    tmul =0.9
    tnum = int(((n*4*rstop)**mod_mul) / (lin_sieve_size))
    ...
    while max_bits < start_bits+max_bits_inc:
        ...
        quad_threshold=round(cmod**quad_threshold_mul)
        if g_debug > 0:
            duration = default_timer() - start
        ...
        start=default_timer()
        interval,quad_co_list,lin_co_list,index_list,mod_list=construct_interval(n,cmod,cfact,qli,primeslist1,hmap,gathered_quad_interval,gathered_ql_interval,rstart,log_map,quad_threshold)
        if g_debug > 0:
        ...
        start=default_timer()
        process_interval(procnum,return_dict,interval,n,quad_co_list,lin_co_list,primeslist1,ret_array,cfact,partials,large_prime_bound,quad_interval,index_list,threshold_map,mod_list)
        if g_debug > 0:
            duration = default_timer() - start
