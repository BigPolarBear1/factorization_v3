...
def launch(lists,n,primeslist1,bmap):
    ...
    hmap,tmod,fast_ind_map=create_hashmap(lists[0],n)
    duration = default_timer() - start
    print("\n[i]Creating iN datastructure in total took: "+str(duration))
    z=0
    print("[*]Launching attack")

    part=(sieve_interval+1)//workers
    rstart=0#round(n**0.5)
    rstop=part#+round(n**0.5)
    if rstart == rstop:
        rstop+=1
    while z < workers:
        p=multiprocessing.Process(target=find_comb, args=(lists[0],n,procnum,return_dict,rstart,rstop,primeslist1,bmap,hmap,fast_ind_map))
        rstart+=part  
        rstop+=part  
        jobs.append(p)
        p.start()
...
def create_hashmap(lists,n):
    ...
    while i < len(lists):
        ...
        j=0
        while j < lists[i]:
            fast_ind_map[-1].append([None]*lists[i])
            hmap[-1].append([])
            k=0
            while k < len(lists[i+1]):
                ...
                l=0
                while l < len(xl):
                    fast_ind_map[-1][-1][xl[l]]=len(hmap[-1][-1])
    ...
    return hmap ,tmod,fast_ind_map

def lookup(lists,alpha,fast_ind_map,iN,hmap):
    result=[]
    index_list=[]
    mod=1
    i=0
    while i < len(hmap):
       # il=hmap[i][iN%lists[i*2]]
        test=None
        test=fast_ind_map[i][iN%lists[i*2]][alpha%lists[i*2]]
        if test !=None:
            res=hmap[i][iN%lists[i*2]][test]
            
            result.append(res)
            mod*=lists[i*2]
        index_list.append(test)
        i+=1

    return result,mod,index_list
...
def check(index_list,fast_ind_map,iN,hmap,lists):
    i=0
    mod=1
    result=[]
    while i < len(index_list):
        if index_list[i] == None:
            i+=1
            continue
        res=hmap[i][iN%lists[i*2]][index_list[i]]
        if len(res)>0:
        #    print(res)
            result.append(res)
            mod*=lists[i*2]
...
def fast_sqr_test(sqr,primeslist1,amount):
    i=0
    while i < amount and i < len(primeslist1):
        res=legendre(sqr,primeslist1[i])
        if res != 1 and res != 0:
            return 0
        i+=1
    return 1

def check_square(res,mod,primeslist1):
    i=0
    test=0
    while i < len(res):
        test+=res[i][0]
    ...
    test%=mod
    test=test**2%mod
    if fast_sqr_test(test,primeslist1,15):
        test_sq=isqrt(test)
       # print("checked")
        if test_sq**2 == test:
            return test_sq
...
def create_enum_check(index_list,fast_ind_map,iN,hmap,lists):
  #  print("iN: ",iN)
  #  print("index_list: ",index_list)
    ...
    while i < len(index_list):
        if index_list[i] == None:
            ...
            continue
        enum.append([])
        enum[-1]=[None]*lists[i*2]
        j=0
        while j < len(hmap[i]):
            res=hmap[i][j][index_list[i]]
            #print("p: "+str(lists[i*2])+" j: "+str(j)+" res: "+str(res))
...
def find_comb(lists,n,procnum,return_dict,rstart,rstop,primeslist1,bmap,hmap,fast_ind_map):
    iN=rstart
    ch=0
    ch2=0
    #print("starting")
    ...
    while iN < rstop:
        alpha=1
        while alpha < sqn:
            res,mod,index_list=lookup(lists,alpha,fast_ind_map,iN,hmap)
           # print("res: ",res)
            #print("a")
            alpa_squared=alpha**2
            if alpa_squared < mod:
                ceil=math.floor((mod - alpa_squared)/n)
                if ceil > 0:
                    i=1
                    enum=create_enum_check(index_list,fast_ind_map,(iN-i)%mod,hmap,lists)
                    while i < round(ceil**0.5):
                        x=0
                        skip=0
                        while x < len(enum):
                            if enum[x] != None:
                                if enum[x][(iN-i)%lists[x*2]]==None:# not in enum[x]:
                                    skip=1
                                    break
                            x+=1
                        ...
                        if skip ==1:
                            ...
                            continue
                        ch2+=1
                        res2,mod2=check(index_list,fast_ind_map,(iN-i)%mod,hmap,lists)

                        if mod != mod2:
                            print("wtf")
                        ch+=1
                        #if mod2 > alpa_squared + n*i:
                        fin=check_square(res2,mod2,primeslist1)
                        if fin != -1:
                            print("square relation: "+str(fin)+" and: "+str(alpha)+" mod: "+str(mod)+" iN: "+str(iN)+" i: "+str(i))
                                #print("res2: ",res2)
                            print("ch: "+str(ch)+" ch2: "+str(ch2))#+" ceil: "+str(ceil))
                            if fin**2%n != alpha**2%n:
                                print("This shouldnt happen")
                            gcd_result=gcd(fin+alpha,n)
...
def init(n,primeslist1):    
    ...
    while 1:
        ...
        if hit ==0:
            break 
    print("lists: ",lists)    
    ###To do not using bmap, delete it
