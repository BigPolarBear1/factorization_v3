...
def check_square(res,mod):
    ...
    test%=mod
    test=test**2%mod
    ##We can run this through a couple of legenre symbols first to test if its even worth checking its square
    ##This is causing a major bottleneck in this version... FIX IT.
    test_sq=isqrt(test)
    if test_sq**2 == test:
        return test_sq
    return -1
        ...
        while alpha < 1000:
            ...
            if alpa_squared < mod:
                ...
                if ceil > 0:
                    ...
                    ##To do: there is a bunch of i values we can just skip...
                    ##There might also be some math here we can use to eliminate or reduce this loop..
                    while i < ceil+1:
                        res2,mod2=check(index_list,fast_ind_map,(iN-i)%mod,hmap,lists)
                        ##Fix this later, they dont have to be equal, can truncate
                        if mod == mod2:
                        #if mod2 > alpa_squared + n*i:
                            fin=check_square(res2,mod2)
                            if fin != -1:
                                print("square relation: "+str(fin)+" and: "+str(alpha))#+" mod: "+str(mod))
