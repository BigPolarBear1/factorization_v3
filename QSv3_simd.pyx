...
cdef Py_ssize_t modinv(Py_ssize_t n,Py_ssize_t p):
    n = n % p
    cdef Py_ssize_t x =0
    cdef Py_ssize_t u = 1
    while n:
...
cdef temp_split(int prime,int prime_index,indexmap,int quad_co,lin_co,cmod,hmap,cfact):
    if prime<min_prime or prime in cfact: ###too slow
        return -1,0
    modi=modinv(cmod%prime,prime)
    k=indexmap[prime_index][quad_co%prime]
    co2=hmap[prime_index][0][k][1]
    res=(co2-lin_co)%prime
    cdef int dist1=(res*modi)%prime
    co2=(prime-co2)%prime
    res=(co2-lin_co)%prime
    cdef int dist2=(res*modi)%prime
...
@cython.boundscheck(False)
@cython.wraparound(False)
cdef create_lin_co(lin_list,cfact,local_mod):
    j=0
    lin=0
    ...
    while len(ret_array[0]) < round((base+1)*matrix_mul):
        ...
        while i < (rstop-1):#gathered_quad_interval_len:
            ...
            if local_mod>1:
                ...
                lin=create_lin_co(lin_list,cfact,local_mod)
...
def find_comb(n,procnum,return_dict2,hmap,primeslist1,indexmap):
    ...
    seen=[]
    start=default_timer()
    quad_interval,threshold_map,quad_interval_index,gathered_quad_interval,gathered_ql_interval=construct_quad_interval(hmap,primeslist1,1,(quad_sieve_size*workers)+1,n)
    if g_debug > 0:
        duration = default_timer() - start
