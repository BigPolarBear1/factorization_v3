...
cdef construct_interval_2(quad_co,lin_co,cmod,primeslist,hmap,n,indexmap,unsigned int [::1] temp,logmap,size,lprimes):
    ...
    cdef Py_ssize_t i=0
    cdef Py_ssize_t k
    cdef Py_ssize_t length = len(lprimes)
    local_primes=[]
    while i < length:
        prime_index=lprimes[i]
        prime=primeslist[prime_index]
        local_primes.append(prime)#to do: change me
        lcmod=cmod%prime
...
cdef sieve_quads(cfact,n,local_mod,quad_interval,quad_interval_index,quad_co):
    ...
    while j < length:
        if quad_interval[i][j] in cfact:
            ...
            local_primes.append(quad_interval_index[i][j])  ###To do: Since we're dealing with primes, we can optimize this using a unique set... 
        j+=1
    if hit == len(cfact):
        found=1
        ...
        while i < 1: #To do: remove or use for multithreading?
            ...
            if local_mod>1:
                ...
                while c < quad_sieve_size+1:
                   # csqrt=math.isqrt(c)
                    if c%2 ==0: ##I don't know, we could just skip precomputing these all together I guess
                        c+=1
                        continue
                    ...
                    while poly_ind < end:
                        ...
                        single_interval,local_primes=construct_interval_2(quad,lin,local_mod,primeslist,hmap,n,indexmap,target,logmap,size,lprimes)
                        ...
                        if mod_found > 10:  ##I don't know what the optimal value is here? 
