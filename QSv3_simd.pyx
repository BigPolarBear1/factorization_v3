...
base=1_000
cdef int lin_sieve_size=1 ### Will be multiplied by chunk size
quad_sieve_size=10
...
chunk_size=5120  ##Total lin_sieve_size = chunk_size*lin_sieve_size
...
def compute_sieve_rows(primeslist1):
    sieve_rows=[]
    i=0
    while i < len(primeslist1):
        prime=primeslist1[i]
      #  if prime > chunk_size:
         #   print("Error, chunk_size smaller then prime, increase chunk size, exiting: ")
        #    return 0
        log=round(math.log2(prime))  ##Precompute this
        rem=chunk_size%prime
        length=chunk_size+prime+(prime-rem)
        ...
        sieve_rows.append(sieve_row_b)        
        i+=1
    return sieve_rows

def launch(n,primeslist1):
    global lin_sieve_size
    sieve_rows=compute_sieve_rows(primeslist1)
    lin_sieve_size=lin_sieve_size*(chunk_size)
    ...
    find_comb(n,z,return_dict2,complete_hmap,primeslist1,indexmap,sieve_rows)
...
@cython.boundscheck(False)
@cython.wraparound(False)
cdef temp_split(int prime,int prime_index,indexmap,cnp.int16_t [::1] sieve_row_b,int quad_co,lin_co,cmod,hmap,cfact):
    if prime<min_prime or prime in cfact:
        return -1,0,0,0,0
    k=indexmap[prime_index][quad_co%prime]
    cdef int row_size=len(sieve_row_b)
    ...
    sieve_row1=sieve_row_b[(prime-dist1):row_size-dist1]
    cdef int size1=row_size-prime
    sieve_row2=sieve_row_b[(prime-dist2):row_size-dist2]
  
    return dist1,dist2,sieve_row1,sieve_row2,size1


@cython.boundscheck(False)
@cython.wraparound(False)
cdef miniloop(int lim,cnp.int16_t [::1] temp,int size1,cnp.int16_t [::1] sieve_row1):
    done=0
    cdef cnp.int16_t [::1] dest
    while done < lim:
        dest=temp[done:done+size1]
        simd_1(dest,sieve_row1)
        done+=size1
    return

@cython.boundscheck(False)
@cython.wraparound(False)
cdef temp_split2(int dist1,int dist2,cnp.int16_t [::1] sieve_row1,cnp.int16_t [::1] sieve_row2,cnp.int16_t [::1] temp,int size1,int lim):
    miniloop(lim,temp,size1,sieve_row1)
    if dist1 != dist2:
        miniloop(lim,temp,size1,sieve_row2)
    return

@cython.boundscheck(False)
@cython.wraparound(False)
cdef construct_interval_simd(quad_co,lin_co,cmod,list quad_interval,list quad_interval_index,list hmap,n,list indexmap,list sieve_rows,cnp.int16_t [::1] temp,list cfact):
    ...
    cdef Py_ssize_t i=0
    while i < length:
        dist1,dist2,sieve_row1,sieve_row2,size1=temp_split(quad_interval[i],quad_interval_index[i],indexmap,sieve_rows[quad_interval_index[i]],quad_co,lin_co,cmod,hmap,cfact)
        if dist1 == -1:
            i+=1
            continue
        temp_split2(dist1,dist2,sieve_row1,sieve_row2,temp,size1,lim)
        i+=1
    return temp
...
@cython.boundscheck(False)
@cython.wraparound(False)
cdef construct_interval(int procnum,list ret_array,partials,n,primeslist,hmap,gathered_quad_interval,gathered_ql_interval,rstart,rstop,quad_interval,quad_interval_index,threshold_map,indexmap,seen,large_prime_bound,sieve_rows,tmul,tnum_list,lprimes_list,tnum_bit_list):
    ...
    while len(ret_array[0]) < round((base+1)*matrix_mul):
        ...
        while i < (rstop-1):#gathered_quad_interval_len:
            ...
            if local_mod>1:
                ...
                single_interval=construct_interval_simd(quad,lin,local_mod,quad_interval[i],quad_interval_index[i],hmap,n,indexmap,sieve_rows,target.copy(),cfact)
...
def find_comb(n,procnum,return_dict2,hmap,primeslist1,indexmap,sieve_rows):
   ...
   # print(len(gathered_quad_interval))
    ...
    ret_array=construct_interval(0,ret_array,partials,n,primeslist1,hmap,gathered_quad_interval,gathered_ql_interval,1,quad_sieve_size+1,quad_interval,quad_interval_index,threshold_map,indexmap,seen,large_prime_bound,sieve_rows,tmul,tnum_list,lprimes_list,tnum_bit_list)
    ...
    #construct_interval(procnum,return_dict,ret_array,partials,n,primeslist1,hmap,gathered_quad_interval,gathered_ql_interval,rstart,quad_interval,quad_interval_index,threshold_map,indexmap,seen,large_prime_bound,sieve_rows,tmul,tnum_list,lprimes_list,tnum_bit_list)
