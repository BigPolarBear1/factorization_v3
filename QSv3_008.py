...
###WORK IN PROGRESS!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
###Factorization_v3_minor_version_007
...
g_limit_mult=2  #lower bound for total modulus of partial resutls, is exponential multiplier..
...
def launch(lists,n,primeslist1):
    ...
    if rstart == rstop:
        rstop+=1
    while z < workers:
        p=multiprocessing.Process(target=find_comb, args=(lists[0],n,procnum,return_dict,rstart,rstop,primeslist1,hmap,fast_ind_map))
        rstart+=part  
        rstop+=part  
        jobs.append(p)
        p.start()
...
def lookup(lists,alpha,fast_ind_map,hmap):
    result=[]
    iN_list=[]
    index_list=[]
    mod=1
    ...
    while i < len(hmap):
       # il=hmap[i][iN%lists[i*2]]
        ...
        while j < len(hmap[i]):
            test=None
            test=fast_ind_map[i][j][alpha%lists[i*2]]
            if test !=None:
                res=hmap[i][j][test]
                iN=create_partial_results_small(lists,j,lists[i*2])
                result[-1].append(res)
                iN_list[-1].append(iN)
                mod*=lists[i*2]
            j+=1      
            index_list[-1].append(test)
        i+=1
 #   print("result: ",result)
 #   print("iN_list: ",iN_list)
 #   print("index_list: ",index_list)
    for a in itertools.product(*iN_list):
        tot=0
        for l in a:
            tot+=l
        tot%=mod
       # print("tot: ",tot)
        break
  #  time.sleep(100000)
    return result,mod,index_list,tot
...
def find_comb(lists,n,procnum,return_dict,rstart,rstop,primeslist1,hmap,fast_ind_map):
    ...
    while alpha < rstop:
        ...
        while iN2 < 100000:  
            ...
            cart_prod_size=1
            while i < len(hmap):
                test=hmap[i][rel_iN%lists[i*2]][index_list[i][iN%lists[i*2]]]
                if len(test) == 0:
                    bad=1
                    break
                coefficients.append(test)
                cart_prod_size*=len(test)
                i+=1
            if bad ==0:
                print("["+str(procnum)+"]rel i-value: "+str(iN2)+" y1: "+str(alpha)+" Cartesian Product size: "+str(cart_prod_size))
                start= default_timer()
                for a in itertools.product(*coefficients): ####Note: We can have a smaller cartesian product by adjusting to a prime field or using larger primes (or lifting them).
                    tot=0
                    for l in a:
                        tot+=l
                    tot%=mod
                    ...
                    while scalar < 100:
                        alpha2sqr%=n 
                        gcd_result=gcd(alpha2sqr+alpha,n)
                        if gcd_result != 1 and gcd_result != n:
                            print("["+str(procnum)+"]Found at scalar: ",scalar)
                            results=[]
                        ...
                        scalar+=1
                duration = default_timer() - start
                print("\n["+str(procnum)+"]Cartesian product took: "+str(duration))
            iN2+=1
        alpha+=1
    return 0
