...
g_limit_mult=10000000#000#000#000   #size of the modulus
xN_limit=20
...
def launch(lists,n,primeslist1,bmap):
    ...
    part=(xN_limit+1)//workers
    rstart=0
    rstop=part
    if rstart == rstop:
        rstop+=1
    while z < workers:
        p=multiprocessing.Process(target=find_comb, args=(lists[0],n,procnum,return_dict,rstart,rstop,primeslist1,bmap))
        rstart+=part  
        rstop+=part  
        jobs.append(p)
        p.start()
...
def find_counterpart_square(clist,total2,lim):
...
def find_comb(lists,n,procnum,return_dict,rstart,rstop,primeslist1,bmap):
    ####Need to optimize this code
    results=[]
    a=0
    while a < 100000:
        same,total=collect(bmap,lists,a)
        hmap,tmod=create_hashmap(lists,n)  ##We should move this into the launch function
        check=1
        while check < 5:
            clist,total2=collect_counterpart(same,hmap,lists,check)
            if total == total2:
                lim=total - check*n
                if lim < 0:
                    check+=1
                    continue
                lim=round(lim**0.5)
                res=find_counterpart_square(clist,total2,lim)

                if res < lim and res != -1:
                    r=res**2+check*n
                    r2=r%n
                    #print("a: "+str(a)+" r2: "+str(r2)+" res: "+str(res)+" xN: "+str(check)+" total: "+str(total))
                    try1=a+res
                    result=gcd(try1,n)
