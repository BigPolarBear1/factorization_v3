...
###WORK IN PROGRESS!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
###Early draft for v3 of my factorization work. It is still extremely slow, but we don't need to check all coefficients and roots, I'll address this in the coming days
###TO DO: Fix coefficient lifting
###TO DO: Dont waste time taking the GCD on bad coefficients
...
g_limit_mult=1000   #size of the modulus
    ...
    while 1:
        ...
        for item in check:
            if len(item)>0:
                ...
                print("factors of " +str(n)+" are: "+str(factor1)+" and "+str(factor2))
                for proc in jobs:
                    proc.terminate()
                return 0
        if balive == 0:
    ...
    for a in itertools.product(*[range(len(x)) for x in lenum]):
        ...
        for r in itertools.product(*xenum):
            ...
            if gcdres != 1 and gcdres != n: 
                if gcdres not in results:
                    results.append(gcdres)
