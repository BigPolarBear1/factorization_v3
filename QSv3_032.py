...
###WORK IN PROGRESS!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
###Factorization_v3 minor_version: 032
###notes: use with python3 QSv3_032.py -keysize 50

##To do: This demonstrates lifting, so we know how the exponents for the factors in the smooth candidates.
##Next we need to get rid of testing for smoothness via trial division, since we have all that information already.
##Also need to implement lifting for powers 2, it gets a little more complicated
##And most importantly, need a strategy for combining coefficients. The idea is to combine coefficients such that the factors outside the factor base shrink.
##That should be possible now... give me a few days to address all this.
        ...
        while exp < 10:
            ...
            while i < len(rt):
                ...
                if len(ret)>0:
                    ...
                    all_ret.extend(ret)
                if len(ret)>1:
                    print("Shouldn't hit unless a prime in the factor base is a factor of N?")
                i+=1
            if len(all_ret)==0:
...
def test_smooth_can(to,n,x,primeslist1):
    ...
    while v < len(primeslist1):
        ...
        v+=1
  #  print("co: "+str(to)+" faclist: "+str(faclist)+" z: "+str(x)+str(" rem: ")+str(smooth_can))
...
def find_smooth(lcol,n,x,primeslist1):
    ...
    while i < len(lcol):
        j=0
        while j < len(lcol[i]):
            a=math.log2(4*n*x+lcol[i][j+1][0]**2)
            b=math.log2(lcol[i][j])
            if b > a*0.4:  ###Fix this later, we should be able to get exact values here.
