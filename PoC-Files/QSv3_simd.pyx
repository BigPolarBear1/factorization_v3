...
def launch(n,primeslist1):
    ...
    if rstart == rstop:
        rstop+=1
    while z < workers:
        p=multiprocessing.Process(target=find_comb, args=(n,z,return_dict2,rstart,rstop,complete_hmap,primeslist1,indexmap,sieve_rows))
        rstart+=part  
        rstop+=part  
        jobs.append(p)
        p.start()
...
@cython.boundscheck(False)
@cython.wraparound(False)
def construct_interval_simd(quad_co,lin_co,cmod,quad_interval,quad_interval_index,hmap,n,indexmap,sieve_rows):
    ...
    while i < len(quad_interval):
        ...
        if k!=None:
            sieve_row_b=sieve_rows[prime_index]
            ...
            sieve_row1=sieve_row_b[(prime-dist1):len(sieve_row_b)-dist1]
            done=0
            while done < (lin_sieve_size*2)+1:
                dest=temp[done:done+len(sieve_row1)]
                #if len(dest)==len(sieve_row1): ##Will waste the end of the interval but whatever
                simd_1(dest[::1],sieve_row1[::1])
                done+=len(sieve_row1)

            if dist1 != dist2:
                sieve_row1=sieve_row_b[(prime-dist2):len(sieve_row_b)-dist2]
                done=0
                while done < (lin_sieve_size*2)+1:
                    dest=temp[done:done+len(sieve_row1)]
                   # if len(dest)==len(sieve_row1):
                    simd_1(dest[::1],sieve_row1[::1])
                    done+=len(sieve_row1)
...
def construct_interval(procnum,return_dict,ret_array,partials,n,primeslist,hmap,gathered_quad_interval,gathered_ql_interval,rstart,quad_interval,quad_interval_index,threshold_map,indexmap,seen,large_prime_bound,sieve_rows):
    i=0
    ...
    while i < gathered_quad_interval_len:
        ...
        if local_mod>1:
            ...
            single_interval=construct_interval_simd(quad,lin,local_mod,quad_interval[i],quad_interval_index[i],hmap,n,indexmap,sieve_rows)
...
def find_comb(n,procnum,return_dict,rstart,rstop,hmap,primeslist1,indexmap,sieve_rows):
   ...
   # residue_map=[]
    ...
    while 1:
        start=default_timer()
        construct_interval(procnum,return_dict,ret_array,partials,n,primeslist1,hmap,gathered_quad_interval,gathered_ql_interval,rstart,quad_interval,quad_interval_index,threshold_map,indexmap,seen,large_prime_bound,sieve_rows)
        if g_debug > 0:
            duration = default_timer() - start
