...
##TO DO: IMPORTANT!! Check with the old version if it is roughly pulling the same amount of smooths to make sure there isn't any bugs in the code..
...
@cython.boundscheck(False)
@cython.wraparound(False)
def construct_interval_simd(quad_co,lin_co,mod_list,primeslist,hmap,n,ind2res_map,res_seen):
    ...
    while i < len(ind2res_map):
        ...
        j=0
        while j < len(ind2res_map[i]): #To do: We should keep track of the non-empty ones while constructing the residue map because this is too slow, especialy if only a few end up getting used.
            if ind2res_map[i][j][0]==0:
                break
            rlength=ind2res_map[i][j][0]
            dist=ind2res_map[i][j][1]

            sieve_row=sieve_row_b[(prime-dist):(prime-dist)+lin_sieve_size]
            new=ind2res_map[i][j]#residue_map[i][ind2res_map[i][j]]#np.asarray(residue_map[i][ind2res_map[i][j]],dtype=np.int16)
            z=2
            while z < rlength+1:
                cmod=mod_list[new[z]]
                if cmod%prime == 0: 
                    ...
                    continue
                if g_debug > 1:
                    CAN=(lin_co[new[z]]+int(dist)*cmod)**2+n*4*quad_co[new[z]]

                    if CAN % (cmod*primeslist[i])  != 0:
                        print("new: "+str(new)+" z: "+str(z)+" prime: "+str(prime)+" i: "+str(i)+" j: "+str(j)+" new[z]: "+str(new[z]))
                        print("ind2res_map 0 0: "+str(ind2res_map[0][0]))
                        print("can: "+str(CAN)+" lin: "+str(lin_co[new[z]])+" dist: "+str(dist)+" cmod: "+str(cmod)+" quad: "+str(quad_co[new[z]]))
...
def construct_interval(procnum,n,primeslist,hmap,gathered_quad_interval,gathered_ql_interval,rstart,quad_interval,quad_interval_index,threshold_map,indexmap,seen):
    ...
    i=0
    while i < len(primeslist):
        ind2res_map.append(np.zeros((8,8),dtype=np.int16)) ###To do: There's a bunch of indexes that don't ever get used. We need to figure out the math for this
        res_seen.append(np.empty(primeslist[i]+1,dtype=np.int16))
        res_seen[-1][::1]=-1
        res_seen[-1][0]=0
        i+=1
    all_interval=[]
    ...
    #print("gathered_quad_interval_len: ",gathered_quad_interval_len)
    while i < gathered_quad_interval_len:
        #print("gathered_ql_interval[i]: ",gathered_ql_interval[i])
        max_bits_inc=50
        tmul =0.9
        tnum = int(((n*2*4*(i+rstart))**mod_mul) / (lin_sieve_size//2))
        if keysize < 99:
            tnum = int(((n*4*(i+rstart))**mod_mul) / (1))
            tmul=2
        start_bits=int(tnum.bit_length() * tmul)
       # print("start_bits: "+str(start_bits)+" tnum: "+str(tnum.bit_length()))
        max_bits =start_bits
        primesl=[]
        g=0
      ...
      #  else: 
       ...
       # if g_debug > 0:
          #  duration = default_timer() - start
          #  print("["+str(procnum)+"]generate_modulus took: "+str(duration))
        if local_mod>1:
            quad=rstart+i 
            ...
            for comb in itertools.product(*enum2):         
              ...
              #  start = default_timer()
                create_residue_map(primeslist,rstart,lin,quad_interval[i],quad_interval_index[i],len(quad_co_list),hmap,quad,local_mod,ind2res_map,indexmap,res_seen)
               # if g_debug > 0:
                #    duration = default_timer() - start
                #    print("["+str(procnum)+"]create_residue_map took: "+str(duration))
                quad_co_list.append(quad)
           # if g_debug > 0:
              #  duration2 = default_timer() - start2
              #  print("["+str(procnum)+"]Loop took: "+str(duration2))
        i+=1
    if len(quad_co_list) > 0:
        ##If we have a lot of primes but 0 quad_co_list len, this still takes way too long. We can optimize this here.
        start = default_timer()
        all_interval=construct_interval_simd(quad_co_list,lin_co_list,mod_list,primeslist,hmap,n,ind2res_map,res_seen)
        if g_debug > 0:
            duration = default_timer() - start
            ...
            if interval[i][j] > threshold:
                ...
                if value != 1:
                    if value < large_prime_bound:
                        ...
                        continue
                #print("found")
               # print("found at quad: ",quad_co_list[i])
...
@cython.boundscheck(False)
@cython.wraparound(False)
def create_residue_map(primeslist1,rstart,lin,quad_interval,quad_interval_index,index,hmap,index2,cmod,ind2res_map,indexmap,res_seen):
   # TO DO: move to a small number and debug all of this the proper way....
    ##To do: Rework this...
    cdef Py_ssize_t j=0
    cdef Py_ssize_t k=0
    while j < len(quad_interval): 
        prime_index=quad_interval_index[j]
        prime=primeslist1[prime_index]
        if prime<min_prime or cmod%prime ==0:
            j+=1
            continue
        k=indexmap[prime_index][index2%prime]
        if k!=None:
            co2=hmap[prime_index][0][k][1]
            res=(co2-lin)%prime

            dist1=solve_lin_con(cmod,res,prime)
            ch=res_seen[prime_index][dist1+1]
            if ch==-1:
                width_end=res_seen[prime_index][0]
                if width_end == np.size(ind2res_map[prime_index],0):

                    temp=np.zeros((np.size(ind2res_map[prime_index],0),np.size(ind2res_map[prime_index],1)),dtype=np.int16)
                    ind2res_map[prime_index]=np.vstack((ind2res_map[prime_index],temp))
                res_seen[prime_index][dist1+1]=width_end
                res_seen[prime_index][0]+=1
                ind2res_map[prime_index][width_end][1]=dist1
                ind2res_map[prime_index][width_end][0]+=2
                end=ind2res_map[prime_index][width_end][0]
                if end > np.size(ind2res_map[prime_index],1)-1:
                    temp=np.zeros((np.size(ind2res_map[prime_index],0),np.size(ind2res_map[prime_index],1)),dtype=np.int16)
                    ind2res_map[prime_index]=np.hstack((ind2res_map[prime_index],temp))
                ind2res_map[prime_index][width_end][end]=index                         
            else:
                ind2res_map[prime_index][ch][0]+=1
                end=ind2res_map[prime_index][ch][0]
                if end > np.size(ind2res_map[prime_index],1)-1:
                    temp=np.zeros((np.size(ind2res_map[prime_index],0),np.size(ind2res_map[prime_index],1)),dtype=np.int16)
                    ind2res_map[prime_index]=np.hstack((ind2res_map[prime_index],temp))
                ind2res_map[prime_index][ch][end]=index
            ...
            if dist2 == dist1:
                j+=1
                continue
            ch=res_seen[prime_index][dist2+1]
            if ch==-1:
                width_end=res_seen[prime_index][0]
                if width_end == np.size(ind2res_map[prime_index],0):
                    ...
                    temp=np.zeros((np.size(ind2res_map[prime_index],0),np.size(ind2res_map[prime_index],1)),dtype=np.int16)
                    ind2res_map[prime_index]=np.vstack((ind2res_map[prime_index],temp))
                res_seen[prime_index][dist2+1]=width_end
                res_seen[prime_index][0]+=1
                ind2res_map[prime_index][width_end][1]=dist2
                ind2res_map[prime_index][width_end][0]+=2
                end=ind2res_map[prime_index][width_end][0]
                if end > np.size(ind2res_map[prime_index],1)-1:
                    temp=np.zeros((np.size(ind2res_map[prime_index],0),np.size(ind2res_map[prime_index],1)),dtype=np.int16)
