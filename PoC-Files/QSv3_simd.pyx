...
base=1_000
lin_sieve_size=1 ### Will be multiplied by chunk size
quad_sieve_size=10
...
@cython.boundscheck(False)
@cython.wraparound(False)
cdef construct_interval_simd(quad_co,lin_co,cmod,quad_interval,quad_interval_index,hmap,n,indexmap,sieve_rows,temp,cfact):
    ...
    while i < len(quad_interval):
        ...
        sieve_row_b=sieve_rows[prime_index][::1]
        co2=hmap[prime_index][0][k][1]
        res=(co2-lin_co)%prime
        dist1=pow(cmod,-1,prime)*res%prime 
        co2=(prime-co2)%prime
        res=(co2-lin_co)%prime
        dist2=pow(cmod,-1,prime)*res%prime 
        sieve_row1=sieve_row_b[(prime-dist1):len(sieve_row_b)-dist1:1]
        done=0
        while done < (lin_sieve_size*2)+1:
            dest=temp[done:done+len(sieve_row1)]
            simd_1(dest[::1],sieve_row1[::1])
            done+=len(sieve_row1)
        if dist1 != dist2:
            sieve_row1=sieve_row_b[(prime-dist2):len(sieve_row_b)-dist2:1]
            done=0
            while done < (lin_sieve_size*2)+1:
                dest=temp[done:done+len(sieve_row1)]
                simd_1(dest[::1],sieve_row1[::1])
