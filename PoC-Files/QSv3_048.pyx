...
###WORK IN PROGRESS!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
###Factorization_v3 minor_version: 048
...
def construct_interval(co,mod,collected,M,hmap,n,iN,log_map):
    interval_size = 2 * M + 1
    sieve_values = [0] * interval_size
    i=0
    while i < len(collected):
        if mod%collected[i][0][0]==0:
            i+=1
            continue
        j=0
...
def process_interval(procnum,return_dict,sieve_values,primeslist1,co,mod,n,iN,ret_array,threshold,collected_plist):
    ##TO DO: Run some checks to see if pimeslist1 can be replaced by colllected primes.. it should.
    i=0
    while i < len(sieve_values):
        if sieve_values[i]>threshold:
            ...
            if value ==1:
                ret_array[0].append(can)
                ret_array[1].append(co+mod*i)
        ...
        while max_bits < start_bits+max_bit_inc:
            ...
            for comb in itertools.product(*enum):
                ...
                for l in comb:
                    tot+=l
                tot%=cmod
                sieve_values=construct_interval(tot,cmod,collected,M,hmap,n,totali,log_map)
                process_interval(procnum,return_dict,sieve_values,primeslist1,tot,cmod,n,totali,ret_array,threshold,collected_plist)
            seen.append(cmod)
        totali+=1
...
def main(lkeysize,lworkers,debug,lbase,lkey):
    global key
    global base
    global workers
    base=lbase
